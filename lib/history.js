...
module.exports = function historyPlugin(schema, options) {
    ...
    schema.post( 'init', function() {
        if(this.toObject()) docid = this.toObject()['_id'];
    });
    ...
    function processUpdate(next) {
        ...
        d._id = this._conditions.$and[0]._id;
        let historyDoc = createHistoryDoc(d, 'u');
        saveHistoryModel(this._conditions, d, historyDoc, this.mongooseCollection.collectionName, next);
    }

    function processRemove(next) {
        let d = {};
        d._id = this._conditions.$and[0]._id;
